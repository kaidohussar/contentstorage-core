...
function getMergedObjects(
  ...
  const getKeyType = (key: any) => {
    const typesOfKey = typeObjects
      .filter((typeObj) => {
        return Object.keys(typeObj as any).includes(key);
      })
      .map((typeObj: any) => typeObj[key])
      .filter(onlyUnique);
...
function getAllUsedTypeIds({ rootTypeId, types }: TypeStructure): string[] {
  ...
  const subTypes = (typeDesc: TypeDescription) => {
    switch (getTypeDescriptionGroup(typeDesc)) {
      case TypeGroup.Array:
        const arrSubTypes = (typeDesc.arrayOfTypes as any)
          .filter(isHash)
          .map((typeId: any) => {
            const typeDesc = types.find((_) => _.id === typeId);
            return subTypes(typeDesc as any);
          })
  ...
  return subTypes(typeDesc as any) as any[];
}
